...
document.addEventListener("DOMContentLoaded", function () {
    ...
    async function enviarDatos(data, btn) {
        try {
            ...
            // 2. LIMPIEZA: Guardar en IndexedDB
            try {
            ...
            // 3. OBTENER NÚMERO DE ENTRADA (SIN no-cors)
            const timestamp = Date.now();
            const response = await fetch(`https://script.google.com/macros/s/AKfycbzJ7187ZwgSY1H2ik0F9L9NJ27yGt2VvLKSOploFG1xhKWXDuuen6PqdzHQpkNyHx7R/exec?getNumeroEntrada&_=${timestamp}`);
            ...
            if (d.numeroEntrada == null) {
                throw new Error("El servidor no devolvió 'numeroEntrada'");
            }
            
            const numeroAsignado = d.numeroEntrada;
            alert(`✅ Número de entrada asignado: ${numeroAsignado}`);
            
            // 5. LIMPIEZA TOTAL DEL FORMULARIO
            ...
            document.getElementById("numero_entrada").value = numeroAsignado;
            ...
            alert(`❌ Error al enviar: ${err.message || "Error desconocido. Verifica la consola para más detalles."}`);
        } finally {
            btn.disabled = false;
...
(() => {
    // Inicializar número de entrada SOLO si no venimos de un envío correcto
    if (!sessionStorage.getItem('formEnviadoOK')) {
        const initNumeroEntrada = async () => {
            try {
                const timestamp = Date.now();
                const response = await fetch(`https://script.google.com/macros/s/AKfycbzJ7187ZwgSY1H2ik0F9L9NJ27yGt2VvLKSOploFG1xhKWXDuuen6PqdzHQpkNyHx7R/exec?getNumeroEntrada&_=${timestamp}`);
                
                if (!response.ok) {
                    console.error("Error HTTP al inicializar número:", response.status);
                    return;
                }
                ...
                if (d.numeroEntrada != null) {
                    ...
                    console.error("Respuesta sin numeroEntrada:", d);
                }
            } catch (error) {
                console.error("❌ Error al inicializar número de entrada:", error);
